...
Upgraded in Batch 6 of the Blender detail pass — every Progenitor
...
Per-asset sockets:
    Structures:
        SOCKET_ResearchPoint  — where the player stands to study
        SOCKET_ApexCrest      — top of the asset (for signal VFX)
    Artifacts:
        SOCKET_PickupPoint    — where the player grabs it
...
def gen_power_core():
    ...
    for sx in [-1, 1]:
        for sy in [-1, 1]:
            ...
            _add(cut, dark_mat)
    bpy.ops.mesh.primitive_uv_sphere_add(radius=0.7, subdivisions=3, location=(0, 0, 1.2))
    _add(bpy.context.active_object, glow_mat)
    for axis in range(3):
...
def gen_data_archive():
    """Crystalline cluster on a hex pedestal — varied crystal heights + central heart."""
    clear_scene()
    dark_mat = palette_material("DarkStone")
...
def gen_resonance_chamber():
    """Buried hexagonal entry portal — frame walls + recessed door + violet glow."""
    clear_scene()
    stone_mat = palette_material("ProgenitorStone")
    dark_mat = palette_material("DarkStone")
...
def main():
    print("\n=== Quiet Rift: Enigma — Remnant Asset Generator (Batch 6 detail upgrade) ===")
